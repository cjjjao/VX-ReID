...
from Baseline import embed_net,Moco_net
from utils import *
import pdb
import scipy.io

parser = argparse.ArgumentParser(description='PyTorch Cross-Modality Training')
parser.add_argument('--dataset', default='RX01', help='dataset name: regdb or sysu]')
parser.add_argument('--lr', default=0.1 , type=float, help='learning rate, 0.00035 for adam')
parser.add_argument('--optim', default='sgd', type=str, help='optimizer')
parser.add_argument('--arch', default='resnet50', type=str, help='network baseline: resnet50')
parser.add_argument('--resume', '-r', default='trial_416_best.t', type=str, help='resume from checkpoint')
...
if args.method =='base':
    ...
    net = Moco_net(n_class, arch=args.arch)
net.to(device)    
...
if dataset == 'sysu':
    ...
    query_feat_pool, query_feat_fc = extract_query_feat(query_loader) #, Xquery_feat_pool, Xquery_feat_fc
    ...
    query_feat_pool, query_feat_fc = extract_query_feat(query_loader)  # , Xquery_feat_pool, Xquery_feat_fc
    for trial in range(times):
        gall_img, gall_label, gall_cam = process_gallery_RX01(data_path, mode=args.mode, trial=trial)
...
mAP = all_mAP / times
mINP = all_mINP / times
cmc_att = all_cmc_att / times
mAP_att = all_mAP_att / times
mINP_att = all_mINP_att / times
